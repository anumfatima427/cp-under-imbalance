...
def get_conformal_quantile(scores, alpha, default_qhat=np.inf, exact_coverage=False):
    ...
    if exact_coverage:
        ...
        n = len(scores)
        if default_qhat == 'max':
            default_qhat = float(np.max(np.array(scores)))
...
def get_RAPS_scores(softmax_scores, labels, lmbda=.01, kreg=5, randomize=True, seed=0):
    ...
    # Regularization
    y_rank = pi.argsort(1)[range(n), labels] + 1 # Compute softmax rank of true labels y
...
#   Huang, Xi, Zhang, Yao, Qiu & Wei (2024)
#   "Conformal Prediction for Deep Classifier via Label Ranking," ICML
#   arXiv:2310.06430
#========================================

def get_SAPS_scores(softmax_scores, labels, lmbda=0.2, randomize=True, seed=0):
    '''
    Compute SAPS nonconformity scores for TRUE LABELS (Huang et al., ICML 2024).
    
    SAPS replaces all softmax probabilities except the maximum with a constant λ,
    then computes cumulative scores based on label ranking.
    
    Paper formula (Definition 3.1):
        s_SAPS(x, y) = π̂_{(1)}(x) + λ · (o(y; π̂(x)) − 1)
    
    where:
        π̂_{(1)}(x) = max softmax probability
        o(y; π̂(x)) = rank of class y (1-indexed, descending by softmax probability)
        λ > 0 = weight hyperparameter (replaces tail softmax values)
    
    Randomized version (analogous to APS randomization):
        s_SAPS(x, y) = π̂_{(1)}(x) + λ · (o(y) − 1) − U · v(y)
        where v(y) = π̂_{(1)}(x) if o(y)=1, else λ
        and U ~ Unif([0,1])
    
    Inputs:
        softmax_scores: n x num_classes array of softmax probabilities
        labels: length-n array of true class labels
        lmbda: float, constant weight replacing tail softmax probabilities (default 0.2)
        randomize: bool, whether to use randomized scores (default True)
        seed: int, random seed for reproducibility
        
    Output: 
        length-n array of SAPS scores for true labels
    '''
    n = len(labels)
    softmax_t = torch.from_numpy(softmax_scores)
    
    # Get max softmax probability per instance: π̂_{(1)}(x)
    pi_max = softmax_t.max(dim=1).values  # (n,)
    
    # Get ranks: sort descending, then invert to get rank of each class (1-indexed)
    sorted_vals, pi = softmax_t.sort(dim=1, descending=True)
    ranks = pi.argsort(dim=1) + 1  # ranks[i, j] = rank of class j for instance i
    
    # Rank of the true label for each instance
    y_rank = ranks[range(n), labels].float()  # (n,)
    
    # Full SAPS cumulative score: π̂_{(1)} + λ · (o(y) - 1)
    full_scores = pi_max + lmbda * (y_rank - 1)
    
    # The "step contribution" at rank o(y):
    #   - If o(y) = 1 (top class): step = π̂_{(1)}(x)
    #   - If o(y) > 1:             step = λ
    step = torch.where(y_rank == 1, pi_max, torch.tensor(lmbda, dtype=pi_max.dtype))
    
    scores = np.array(full_scores)
    step = np.array(step)
    
    if not randomize:
        # Cumsum excluding y's contribution: full_score - step
        return scores - step
    else:
        np.random.seed(seed)
        U = np.random.rand(n)
        # Partial inclusion of y: full_score - U * step
        return scores - U * step


def get_SAPS_scores_all(softmax_scores, lmbda=0.2, randomize=True, seed=0):
    '''
    Compute SAPS scores for ALL CLASSES (Huang et al., ICML 2024).
    
    Paper formula (Definition 3.1):
        s_SAPS(x, k) = π̂_{(1)}(x) + λ · (o(k; π̂(x)) − 1)
    ...
    Output: 
        n x num_classes array of SAPS scores
    '''
    n = softmax_scores.shape[0]
    softmax_t = torch.from_numpy(softmax_scores)
    
    # Max softmax probability per instance: π̂_{(1)}(x)
    pi_max = softmax_t.max(dim=1).values  # (n,)
    
    # Ranks for all classes (1-indexed, descending)
    sorted_vals, pi = softmax_t.sort(dim=1, descending=True)
    ranks = pi.argsort(dim=1) + 1  # (n, num_classes)
    
    # Full SAPS score for each class: π̂_{(1)} + λ · (rank - 1)
    full_scores = pi_max.unsqueeze(1) + lmbda * (ranks.float() - 1)
    
    # Step contribution: π̂_{(1)} for rank-1 class, λ for all others
    step = torch.where(ranks == 1, pi_max.unsqueeze(1), torch.tensor(lmbda, dtype=pi_max.dtype))
    
    full_scores = np.array(full_scores)
    step = np.array(step)
    
    if not randomize:
        return full_scores - step
    else:
        np.random.seed(seed)
        U = np.random.rand(*softmax_scores.shape)
        return full_scores - U * step


#========================================
#   Tail-Aware Conformal Prediction (TACP / sTACP)
#   Liu, Zeng & Wei (2025)
#   "Conformal Prediction Meets Long-tail Classification"
#   arXiv:2508.11345
#========================================

def get_TACP_scores(base_scores_all, softmax_scores, labels, class_frequencies,
                    lmbda=0.01, eta=0.5, smooth=False, randomize=False, seed=0):
    '''
    Compute TACP or sTACP nonconformity scores for TRUE LABELS (Liu et al., 2025).
    
    TACP wraps any base score and adds a rank-based penalty ONLY to head classes,
    reducing head-conditional coverage to indirectly increase tail-conditional coverage.
    
    Paper formulas:
    
    TACP (binary head/tail):
        s_TACP(x, y) = s_base(x, y) + λ · o(y; π̂(x)) · 𝟙[y ∈ H_η]
    
    sTACP (smooth reweighting):
        s_sTACP(x, y) = s_base(x, y) + λ · o(y; π̂(x)) · w(y)
        where w(y) = n_y / n_max  (smooth frequency weight)
    
    where:
        s_base(x, y) = any base nonconformity score (pre-computed)
        o(y; π̂(x)) = rank of class y (1-indexed, descending by softmax probability)
        H_η = head class set (classes with frequency above the η-th percentile)
        n_y = frequency of class y in training data
        n_max = frequency of the most common class
    
    Inputs:
        base_scores_all: n x num_classes array of pre-computed base scores (e.g., from APS, RAPS)
        softmax_scores: n x num_classes array of softmax probabilities (for computing ranks)
        labels: length-n array of true class labels
        class_frequencies: length-num_classes array of class counts in training data
        lmbda: float, penalty strength (default 0.01)
        eta: float in [0,1], percentile threshold for head/tail partition (default 0.5).
             Classes with frequency >= η-th percentile are "head"
        smooth: bool, if True use sTACP (smooth weights), if False use TACP (binary head/tail)
        randomize: bool, not used (penalty is deterministic); included for API consistency
        seed: int, random seed (not used for TACP penalty itself)
        
    Output: 
        length-n array of TACP/sTACP scores for true labels
    '''
    n = len(labels)
    class_frequencies = np.array(class_frequencies, dtype=float)
    num_classes = len(class_frequencies)
    
    # Extract base score for true label
    if len(base_scores_all.shape) == 2:
        base_scores = base_scores_all[np.arange(n), labels]
    else:
        base_scores = base_scores_all
    
    # Compute ranks from softmax (1-indexed, descending)
    softmax_t = torch.from_numpy(softmax_scores)
    sorted_vals, pi = softmax_t.sort(dim=1, descending=True)
    ranks = np.array(pi.argsort(dim=1) + 1)  # (n, num_classes)
    
    # Rank of true label for each instance
    y_rank = ranks[np.arange(n), labels].astype(float)  # (n,)
    
    if smooth:
        # sTACP: smooth frequency weight w(y) = n_y / n_max
        n_max = np.max(class_frequencies)
        weights = class_frequencies / n_max  # (num_classes,)
        w_y = weights[labels]  # (n,) weight for each instance's true label
        penalty = lmbda * y_rank * w_y
    else:
        # TACP: binary head/tail indicator
        freq_threshold = np.percentile(class_frequencies, eta * 100)
        head_classes = class_frequencies >= freq_threshold  # (num_classes,) boolean
        is_head = head_classes[labels]  # (n,) boolean for each instance's true label
        penalty = lmbda * y_rank * is_head.astype(float)
    
    return base_scores + penalty


def get_TACP_scores_all(base_scores_all, softmax_scores, class_frequencies,
                        lmbda=0.01, eta=0.5, smooth=False):
    '''
    Compute TACP or sTACP scores for ALL CLASSES (Liu et al., 2025).
    
    Paper formulas:
    
    TACP:  s_TACP(x, k) = s_base(x, k) + λ · o(k; π̂(x)) · 𝟙[k ∈ H_η]
    sTACP: s_sTACP(x, k) = s_base(x, k) + λ · o(k; π̂(x)) · w(k)
    
    Inputs:
        base_scores_all: n x num_classes array of pre-computed base scores
        softmax_scores: n x num_classes array of softmax probabilities
        class_frequencies: length-num_classes array of class counts in training data
        lmbda: float, penalty strength (default 0.01)
        eta: float in [0,1], percentile threshold for head/tail partition (default 0.5)
        smooth: bool, if True use sTACP (smooth weights), if False use TACP (binary)
    
    Output: 
        n x num_classes array of TACP/sTACP scores
    '''
    class_frequencies = np.array(class_frequencies, dtype=float)
    num_classes = len(class_frequencies)
    
    # Compute ranks for all classes (1-indexed, descending)
    softmax_t = torch.from_numpy(softmax_scores)
    sorted_vals, pi = softmax_t.sort(dim=1, descending=True)
    ranks = np.array(pi.argsort(dim=1) + 1, dtype=float)  # (n, num_classes)
    
    if smooth:
        # sTACP: w(k) = n_k / n_max for each class k
        n_max = np.max(class_frequencies)
        weights = class_frequencies / n_max  # (num_classes,)
        # Broadcast: ranks (n, K) * weights (K,) -> (n, K)
        penalty = lmbda * ranks * weights[np.newaxis, :]
    else:
        # TACP: 𝟙[k ∈ H_η]
        freq_threshold = np.percentile(class_frequencies, eta * 100)
        head_mask = (class_frequencies >= freq_threshold).astype(float)  # (num_classes,)
        penalty = lmbda * ranks * head_mask[np.newaxis, :]
    
    return base_scores_all + penalty


#========================================
#   Frequency-Aware Regularized Prediction Sets (FRAPS)
#   PROPOSED METHOD
#   Extends RAPS (Angelopoulos et al., 2021) with class-frequency-weighted
#   regularization penalty for imbalanced classification.
#========================================

def get_FRAPS_scores(softmax_scores, labels, class_frequencies,
                     lmbda_base=0.01, kreg=2, beta=1.0, randomize=True, seed=0):
    '''
    Compute FRAPS nonconformity scores for TRUE LABELS.
    
    FRAPS extends RAPS by replacing the uniform regularization penalty λ with a
    class-frequency-dependent penalty λ(y), reducing penalty for rare classes.
    
    Formula:
        s_FRAPS(x, y) = Σ_{k: π̂_k(x) ≥ π̂_y(x)} π̂_k(x) + λ(y) · max(0, o(y) − k_reg)
    
    where:
        λ(y) = λ_base · (n_y / n_max)^β
        
        n_y = frequency of class y in training data
        n_max = frequency of the most common class
        β > 0 controls frequency scaling strength
        
    Special cases:
        β = 0 → λ(y) = λ_base for all y → reduces to standard RAPS
        β → ∞ → λ(y) → 0 for all minority classes → reduces to APS for minorities
    
    For DDR dataset [50%, 5.7%, 26.4%, 1.9%, 16%] with β=1:
        λ(No DR)         = λ_base · 1.000  (full penalty, majority class)
        λ(Mild DR)        = λ_base · 0.114
        λ(Moderate DR)    = λ_base · 0.528
        λ(Severe DR)      = λ_base · 0.038  (near-zero penalty, rarest class)
        λ(Proliferative)  = λ_base · 0.320
    
    Inputs:
        softmax_scores: n x num_classes array of softmax probabilities
        labels: length-n array of true class labels
        class_frequencies: length-num_classes array of class counts in training data
        lmbda_base: float, base regularization strength (default 0.01)
        kreg: int, rank threshold below which no penalty is applied (default 2)
        beta: float, frequency scaling exponent (default 1.0)
        randomize: bool, whether to use randomized scores (default True)
        seed: int, random seed for reproducibility
    
    Output: 
        length-n array of FRAPS scores for true labels
    '''
    n = len(labels)
    class_frequencies = np.array(class_frequencies, dtype=float)
    
    # Compute per-class λ values: λ(y) = λ_base · (n_y / n_max)^β
    n_max = np.max(class_frequencies)
    lmbda_per_class = lmbda_base * (class_frequencies / n_max) ** beta  # (num_classes,)
    
    # APS cumulative score (same as RAPS before regularization)
    softmax_t = torch.from_numpy(softmax_scores)
    sorted_vals, pi = softmax_t.sort(dim=1, descending=True)
    scores = sorted_vals.cumsum(dim=1).gather(1, pi.argsort(1))[range(n), labels]
    
    # Rank of true label (1-indexed, descending)
    y_rank = pi.argsort(1)[range(n), labels] + 1  # (n,)
    
    # Frequency-weighted regularization: λ(y) · max(0, rank(y) - k_reg)
    lmbda_y = torch.from_numpy(lmbda_per_class[labels])  # (n,) λ for each instance's true label
    reg = torch.maximum(lmbda_y * (y_rank.float() - kreg), torch.zeros(size=y_rank.shape))
    scores += reg
    ...
    if not randomize:
        ...
        np.random.seed(seed)
        U = np.random.rand(n)
        return scores - U * softmax_scores[range(n), labels]


def get_FRAPS_scores_all(softmax_scores, class_frequencies,
                         lmbda_base=0.01, kreg=2, beta=1.0, randomize=True, seed=0):
    '''
    Compute FRAPS scores for ALL CLASSES.
    
    Formula:
        s_FRAPS(x, k) = Σ_{j: π̂_j(x) ≥ π̂_k(x)} π̂_j(x) + λ(k) · max(0, o(k) − k_reg)
    
    where λ(k) = λ_base · (n_k / n_max)^β
    ...
    Output: 
        n x num_classes array of FRAPS scores
    '''
    n = softmax_scores.shape[0]
    num_classes = softmax_scores.shape[1]
    class_frequencies = np.array(class_frequencies, dtype=float)
    
    # Compute per-class λ values: λ(k) = λ_base · (n_k / n_max)^β
    n_max = np.max(class_frequencies)
    lmbda_per_class = lmbda_base * (class_frequencies / n_max) ** beta  # (num_classes,)
    
    # APS cumulative scores for all classes
    softmax_t = torch.from_numpy(softmax_scores)
    sorted_vals, pi = softmax_t.sort(dim=1, descending=True)
    scores = sorted_vals.cumsum(dim=1).gather(1, pi.argsort(1))
    
    # Rank of each class (1-indexed, descending)
    y_rank = pi.argsort(1) + 1  # (n, num_classes)
    
    # Frequency-weighted regularization for each class:
    # λ(k) · max(0, rank(k) - k_reg)
    # lmbda_per_class is (num_classes,), broadcast across instances
    lmbda_matrix = torch.from_numpy(
        np.tile(lmbda_per_class, (n, 1))  # (n, num_classes)
    )
    reg = torch.maximum(
        lmbda_matrix * (y_rank.float() - kreg),
        torch.zeros(size=scores.shape)
    )
    scores += reg
    
    if not randomize:
        return np.array(scores) - softmax_scores
    else:
        np.random.seed(seed)
        U = np.random.rand(*softmax_scores.shape)
        return np.array(scores) - U * softmax_scores
