...
def load_dataset(dataset, data_folder='data'):
    ...
    Input:
        - dataset: string specifying dataset. Options are 'ddr'
        - data_folder: string specifying folder containing the <dataset name>.npz files

    Output: softmax_scores, labels
        
    '''
    assert dataset in ['ddr-ce','connah-ddr', 'ddr-focal','ddr-cp-aware','dfuc-ce','dfuc-cp-aware','dfuc-focal','ddr-cp-aware','isic-ce','isic-focal','isic-cp-aware']
...
def run_one_experiment(dataset, save_folder, alpha, n_totalcal, score_function_list, methods, seeds, 
    ...
    # Compute class frequencies from the full label set (needed for FRAPS, TACP, sTACP)
    num_classes = softmax_scores.shape[1]
    class_frequencies = np.array([np.sum(labels == k) for k in range(num_classes)])
    print(f'Class frequencies: {class_frequencies} (total={np.sum(class_frequencies)})')
    ...
    for score_function in score_function_list:
        ...
        # --- NEW SCORE FUNCTIONS ---
            ...
            # Huang et al. (ICML 2024) - Sorted Adaptive Prediction Sets
            lmbda = 0.2  # Default from paper / TorchCP
            scores_all = get_SAPS_scores_all(softmax_scores, lmbda=lmbda, randomize=True)
            
        elif score_function == 'TACP_APS':
            # Liu et al. (2025) - TACP with APS as base score (binary head/tail)
            base_scores_all = get_APS_scores_all(softmax_scores, randomize=True)
            scores_all = get_TACP_scores_all(base_scores_all, softmax_scores, class_frequencies,
                                             lmbda=0.01, eta=0.5, smooth=False)

        elif score_function == 'sTACP_APS':
            # Liu et al. (2025) - sTACP with APS as base score (smooth reweighting)
            base_scores_all = get_APS_scores_all(softmax_scores, randomize=True)
            scores_all = get_TACP_scores_all(base_scores_all, softmax_scores, class_frequencies,
                                             lmbda=0.01, eta=0.5, smooth=True)
            
        elif score_function == 'TACP_RAPS':
            # Liu et al. (2025) - TACP with RAPS as base score (binary head/tail)
            base_scores_all = get_RAPS_scores_all(softmax_scores, lmbda=0.01, kreg=2, randomize=True)
            scores_all = get_TACP_scores_all(base_scores_all, softmax_scores, class_frequencies,
                                             lmbda=0.01, eta=0.5, smooth=False)
            
        elif score_function == 'sTACP_RAPS':
            # Liu et al. (2025) - sTACP with RAPS as base score (smooth reweighting)
            base_scores_all = get_RAPS_scores_all(softmax_scores, lmbda=0.01, kreg=2, randomize=True)
            scores_all = get_TACP_scores_all(base_scores_all, softmax_scores, class_frequencies,
                                             lmbda=0.01, eta=0.5, smooth=True)
        
        elif score_function == 'FRAPS':
            # PROPOSED: Frequency-Aware Regularized Prediction Sets
            lmbda_base = 0.01
            kreg = 2
            beta = 1.0
            scores_all = get_FRAPS_scores_all(softmax_scores, class_frequencies,
                                              lmbda_base=lmbda_base, kreg=kreg, beta=beta, randomize=True)
            
        else:
            raise Exception(f'Undefined score function: {score_function}. '
                          f'Options: softmax, APS, RAPS, SAPS, TACP_APS, sTACP_APS, '
                          f'TACP_RAPS, sTACP_RAPS, FRAPS')
