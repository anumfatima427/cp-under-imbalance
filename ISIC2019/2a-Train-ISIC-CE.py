#!/usr/bin/env python3
"""
ISIC Training Script — Baseline (CrossEntropyLoss) & CP Readiness
- Pre-defined Train/Val splits from separate folders
- Data summary
- Comprehensive Train & Val metric logging per epoch
- Best model tracking in CSV
- Aspect-ratio preserving transforms for lesion morphology
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Baseline ResNet50 for ISIC with CP setup")
    parser.add_argument('--data_dir', type=str, required=True, help="Path to dataset root (e.g. ./datasplit)")
    parser.add_argument('--output', type=str, default='./model_output/isic_baseline', help="Output directory")
...
def main() -> None:
    ...
    # --- Print Data Summary ---
    print("\n" + "="*45)
    print(" DATA SUMMARY (EXPLICIT TRAIN/VAL SPLITS)")
    print("="*45)
    ...
    train_ds = ISICDataset(train_df, train_imgs, train_t)
    val_ds = ISICDataset(val_df, val_imgs, val_t)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if improved:
            ...
            # Save Val data as CP Calibration
    ...
    # Evaluating on VAL set since no test set was provided
    _, final_metrics, final_probs, final_labels, final_preds = eval_epoch(model, val_loader, criterion, device)

    print("\nFinal Metrics (Best Model on Val Set):")
    ...
    np.savez(out_dir / 'cp_val_data.npz', probs=final_probs, labels=final_labels)
    print(f"\n✔ ISIC Pipeline complete. Check `{out_dir.name}` for outputs and CP data.")
