#!/usr/bin/env python3
"""
DDR Training Script — Focal Loss Experiment & CP Readiness
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Focal Loss ResNet50 for DFUC2021 with CP setup")
    parser.add_argument('--data_dir', type=str, required=True, help="Path to dataset root")
    parser.add_argument('--output', type=str, default='./model_output/focal_loss', help="Output directory")
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        # Focal scaling factor: (1 - pt)^gamma
...
def main() -> None:
    ...
    # Swapped CrossEntropy for custom Focal Loss
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if improved:
            ...
            # Update all previous history records to clear older best models
            for h in history:
                h['is_best'] = False
            row['is_best'] = True
    ...
    # Save CSV with the exact 'is_best' boolean per epoch
    ...
    print(f"\n✔ Focal Loss Pipeline complete. Check `{out_dir.name}` for outputs and CP data.")
